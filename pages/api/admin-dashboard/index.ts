      ...
      switch (req.body.type) {
        ...
        case 'VOUCHER_LIST':
          try {
            ...
            const data = await collection.find({}).sort({ voucherDate: -1 }).limit(4).toArray();
        ...
        case 'LEAVE_LIST':
          try {
            ...
            const data = await collection.find({}).limit(3).sort({ createdDate: -1 }).toArray();
        ...
        case 'TASK_LIST':
          try {
            ...
            const data = await collection.find({}).sort({ createdDate: -1 }).limit(5).toArray();
            res.status(200).json(data);
          }
          catch (err) {
            if (err instanceof Error) {
